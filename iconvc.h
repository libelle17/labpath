...
// namespace iconv_wrapper {
  ...
  public:
    ...
    iconvc (const std::string &fromcode, const std::string &tocode)
    {
    ...
    // Enable move
    iconvc (iconvc &&) = default;
    iconvc& operator = (iconvc &&) = default;
  ...
  private:
    ...
    iconvc (iconvc const &) = delete;
    iconvc& operator = (iconvc const &) = delete;
  ...
  inline void iconvc::open (const std::string &fromcode,
  ...
  inline void iconvc::close () noexcept
  {
    if (convdesc != invalid_cd)
      {
  ...
  inline std::string iconvc::convert (const std::string &in)
  {
  ...
  inline std::string iconvc::get_initial_sequence (void)
  {
    std::string out (1, '\0');
  ...
  inline std::string &iconvc::get_initial_sequence (std::string *pout)
  {
    do_iconv (pout, nullptr, nullptr);
    return *pout;
  }

  inline void iconvc::reset (void) const noexcept
  {
    ::iconv (convdesc, nullptr, nullptr, nullptr, nullptr);
  }

  inline void iconvc::do_iconv (std::string *pout,
